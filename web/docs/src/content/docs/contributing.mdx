...
The only thing you need is `npm`, a JavaScript package manager (built on the `node` runtime).
...
Open [http://localhost:4321](http://localhost:4321) in your browser to see the docs.
...
system, run while inside the `web/docs/` directory:
...
[https://docs.jj-vcs.dev/latest](https://docs.jj-vcs.dev/latest), which is a copy of
